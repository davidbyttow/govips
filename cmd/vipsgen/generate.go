...
// Generate creates all C wrapper, C header, and Go bridge files from the
// discovered operations.
func Generate(ops []OpDef, outputDir string) error {
	// Filter to generatable operations and group by category.
	catOps := make(map[string][]OpDef)
	for _, op := range ops {
		if excludeOps[op.Name] {
			continue
		}
		cat := normalizeCategoryForOp(op.Name, op.Category)
		catOps[cat] = append(catOps[cat], op)
	}

	catNames := make([]string, 0, len(catOps))
	for name := range catOps {
	...
	for _, cat := range catNames {
		...
		cCode := genCSource(cat, ops)
		hCode := genCHeader(cat, ops)
		goCode := genGoBridge(cat, ops)

		cPath := filepath.Join(outputDir, fmt.Sprintf("gen_%s.c", cat))
		hPath := filepath.Join(outputDir, fmt.Sprintf("gen_%s.h", cat))
		goPath := filepath.Join(outputDir, fmt.Sprintf("gen_%s.go", cat))
		...
		totalOps += len(ops)
		fmt.Printf("  gen_%s.{c,h,go}: %d operations\n", cat, len(ops))
	}

	// Generate helpers file.
	helpersCode := genGoHelpers()
	helpersPath := filepath.Join(outputDir, "gen_helpers.go")
	if err := writeIfChanged(helpersPath, helpersCode); err != nil {
		return err
	}
	fmt.Printf("  gen_helpers.go\n")

	fmt.Printf("\nGenerated code for %d operations\n", totalOps)
	return nil
}
...
func genCSource(category string, ops []OpDef) []byte {
	var w bytes.Buffer
	fmt.Fprintf(&w, "// Code generated by vipsgen. DO NOT EDIT.\n")
	fmt.Fprintf(&w, "#include \"gen_%s.h\"\n\n", category)
...
func genCHeader(category string, ops []OpDef) []byte {
	var w bytes.Buffer
	guard := "GEN_" + strings.ToUpper(category) + "_H"
	fmt.Fprintf(&w, "// Code generated by vipsgen. DO NOT EDIT.\n")
...
func genGoBridge(category string, ops []OpDef) []byte {
	var w bytes.Buffer
	fmt.Fprintf(&w, "// Code generated by vipsgen. DO NOT EDIT.\n")
...
func genGoHelpers() []byte {
	return []byte(`// Code generated by vipsgen. DO NOT EDIT.
package vips
